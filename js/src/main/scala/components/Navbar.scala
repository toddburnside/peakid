...
object Navbar {
  ...
  private case class MenuItem(idx: Int, label: (Props) => VdomNode, iconName: String,  page: AppPage)
  ...
  def renderMenuItem(props: Props)(item: MenuItem): VdomElement =
    <.li(^.key := item.idx, (^.className := "active").when(props.currentPage == item.page),
      props.router.link(item.page)(nameToIcon(item.iconName), " ", item   .label(props)))

  private class Backend($: BackendScope[Props, Unit]) {
    def render(props: Props) = {
      <.nav(^.className := "navbar navbar-inverse navbar-fixed-top",
        <.div(^.className := "container",
          <.div(^.className := "navbar-header", <.span(^.className := "navbar-brand", "peakid")),
  ...
  private val component = ScalaComponent.builder[Props]("Navbar")
