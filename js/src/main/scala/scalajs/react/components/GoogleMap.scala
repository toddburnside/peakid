...
object GoogleMap {
  ...
  case class State(mapObjects: Option[(GMap, GInfoWindow)],
                   markers: List[GMarker])
        ...
        .map(
          node =>
            t.modState(
              _.copy(
                mapObjects = Some(
                  (new GMap(node, MapOptions(P.center, P.zoom).toGMapOptions),
                   new GInfoWindow))),
  ...
  val component = ScalaComponent
    ...
    .componentWillReceiveProps {
      case componentWillReceiveProps =>
        componentWillReceiveProps.backend.updateMap(
          componentWillReceiveProps.nextProps)
    }
    .componentDidMount($ => $.backend.loadScript($.props))
    .componentWillUnmount($ =>
      Callback($.state.markers.foreach(new GClearInstanceListeners(_))))
    .build
