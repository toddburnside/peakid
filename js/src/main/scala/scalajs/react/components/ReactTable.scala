package scalajs.react.components
...
object ReactTable {
  ...
  type Config =
    (String,
     Option[Any => VdomElement],
     Option[(Model, Model) => Boolean],
     Option[Double])
  ...
  class Backend(t: BackendScope[Props, State]) {
    ...
    def onTextChange(P: Props)(value: String): Callback =
      t.modState(
        _.copy(filteredModels = getFilteredModels(value, P.data), offset = 0))
    ...
    def sort(f: (Map[String, Any], Map[String, Any]) => Boolean,
             item: String): Callback =
      t.modState { S =>
        val rows = S.filteredModels
        S.sortedState.getOrElse(item, "") match {
          case ASC =>
            S.copy(filteredModels = rows.reverse,
                   sortedState = Map(item -> DSC),
                   offset = 0)
          case DSC =>
            S.copy(filteredModels = rows.sortWith(f),
                   sortedState = Map(item -> ASC),
                   offset = 0)
          case _ =>
            S.copy(filteredModels = rows.sortWith(f),
                   sortedState = Map(item -> ASC),
  ...
  def getIntSort(key: String) =
    (m1: Model, m2: Model) =>
      m1(key).asInstanceOf[Int] < m2(key).asInstanceOf[Int]

  def getDoubleSort(key: String) =
    (m1: Model, m2: Model) =>
      m1(key).asInstanceOf[Double] < m2(key).asInstanceOf[Double]

  def getLongSort(key: String) =
    (m1: Model, m2: Model) =>
      m1(key).asInstanceOf[Long] < m2(key).asInstanceOf[Long]
  ...
  def getColumnDiv(key: String, config: List[Config]) = {
    ...
    if (group.nonEmpty && group.head._4.isDefined)
      <.div(^.flex := group.head._4.get.toString)
    else <.div()
  }
                  ...
                  if (f.isDefined) {
                    cell(
                      ...
                      state.sortedState
                        .get(item)
                        .whenDefined(value =>
                          props.style.sortIcon(value == ASC))
//                        .orElse(EmptyVDom)
//                      props.style
...
//                        .when(state.sortedState.isDefinedAt(item))
  ...
  val component = ScalaComponent
    ...
    .initialStateFromProps(p =>
      State(filterText = "", offset = 0, p.rowsPerPage, p.data, Map()))
